...
module.exports = async (req, res, next) => {
  ...
  const checkHeaderLength = await authHeader.split(" ");

  if (checkHeaderLength.length !== 3) {
    return res.status(403).json({
      message: "You are not authorised to access this API!",
      status: 0,
    });
  ...
  const SUPER_SECRET = await authHeader.split(" ")[2];

  // Decrypt Hidden Message
  const bytes = CryptoJS.AES.decrypt(SUPER_SECRET, process.env.SUPER_SECRET);
